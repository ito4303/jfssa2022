...
fit1 <- coda.samples(model, variable.names = "beta",
                     n.iter = 1000)
```
...
fit2 <- nimbleMCMC(model = model, monitors = "beta",
...
cat(occ_stan_model[1:12], sep = '\n')
```

---

```{r occ_stan_model2}
cat(occ_stan_model[14:24], sep = '\n')
```
...
model <- jags.model(nmix_bugs,
                    ...
                    n.chains = 3, n.adapt = 1000)
update(model, n.iter = 1000)
fit4 <- coda.samples(model, variable.names = c("beta", "Ntotal"),
                     n.iter = 1000)
```
...
```{r nmix_stan_model3}
cat(nmix_stan_model[14:27], sep = '\n')
```

---

```{r nmix_stan_model4}
cat(nmix_stan_model[29:32], sep = '\n')
```

---

```{r nmix_stan_model5}
cat(nmix_stan_model[34:41], sep = '\n')
```
