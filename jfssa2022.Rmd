...
- 観測過程: $i$回目の観測では、そのうち$Y_i$個体を、発見確率$p$で発見する
    - $Y_i \sim \mathrm{Binomial}(N, p)$
...
- 各サイトについて
$$
L(Y_{j} \mid \lambda, p_{j}) = \sum_{k=0}^\infty \left(\mathrm{Pois}(k \mid \lambda) \prod_{j=1}^{J}\mathrm{Binom}(Y_{j} \mid k, p_{j})\right)
$$
