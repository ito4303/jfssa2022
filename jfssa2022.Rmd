...
occ_data <- AHMbook::simOcc(
  ...
  alpha1 = -1,            # 占有確率に対する標高の係数
  alpha2 = -3,            # 占有確率に対する風速の係数
  alpha3 = 0,             # 占有確率に対する交互作用の係数
  sd.lp = 0,              # サイトの変量効果
  b = 0,                  # 行動反応
  show.plot = FALSE)
```
...
- 各サイトについて
$$
L(Y \mid \lambda, p_{j}) = \sum_{k=0}^\infty \left(\mathrm{Pois}(k \mid \lambda) \prod_{j=1}^{J}\mathrm{Binom}(Y_{j} \mid k, p_{j})\right)
$$
