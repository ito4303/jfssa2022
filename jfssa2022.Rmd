...
fit <- coda.samples(model, variable.names = "beta",
                    n.iter = 1000)
```

## 結果

```{r occ_jags_summary, echo=11:17}
summary(fit)
```
...
fit <- nimbleMCMC(model = model, monitors = "beta",
...
fit <- model$sample(data = list(M = occ_data$M,
                                J = occ_data$J,
                                Y = occ_data$y,
                                Elev = occ_data$elev,
                                Forest = occ_data$forest,
...
BUGS
```{r nmix_bugs_model, echo=TRUE, cache=TRUE}
...
model <- jags.model(nmix_bugs,
                    ...
                    n.chains = 3, n.adapt = 1000)
update(model, n.iter = 1000)
fit <- coda.samples(model, variable.names = c("beta", "Ntotal"),
                    n.iter = 1000)
```
...
fit <- nimbleMCMC(code, constants = const, data = data, inits = init,
...
fit$summary$all.chains[(M + 1):(M + 5), ]
```
...
df <- data.frame(Site = rep(1:M, 2),
                 N = c(nmix_data$N, fit$summary$all.chains[1:M, 1]),
                 Value = rep(c("True", "Estimated"), each = M))
ggplot(df) +
  geom_segment(data = data.frame(Site = 1:nmix_data$nsite,
                                 CI_low = fit$summary$all.chains[1:M, 4],
                                 CI_upp = fit$summary$all.chains[1:M, 5]),
...
fit <- model$sample(data = list(M = nmix_data$nsites,
                                J = nmix_data$nvisits,
                                Y = nmix_data$C, K = 100,
                                Xsite = nmix_data$site.cov[, 2],
                                Xsurvey = nmix_data$survey.cov),
